...
say _which_ block on _which_ line an `end` actually closes, and hand back the exact text to
write instead.

## `mz outline`: the representation for dependencies

When editing component A that uses B, an agent needs B's _interface_, not B's body. Reading
